...
$trolley = db_moscow_transport::find('all', array(
	'conditions' => array('type="trolley2" AND convertable=1'),
...
<div>
	...
	<input type="button" value="Создать новый маршрут" onClick="gd.createNewRoute();">
...
gd = {
	...
	openFromPoly: function(number){
		...
		if (typeof(gd.current_route) != "undefined")
			gdMap.map.geoObjects.remove(gd.current_route);
		
		gd.plans = [];
		gd.selected.route_id = 0;
		gd.plans[0] = {};
		
		gd.routeName = poly[number].poly[0].t_name + ' resaved';
		
		gd.prepareRouteArray(poly[number].dots);
	...
	onMapInit: function(){
		...
		for (var i = 0; i < poly.length; i++){
			...
			gd.drawPolyline(gd.coords, poly[i][0].t_name);
		}
